...
def invest_percent(balance, trade_number):    """Trade #1 gets full deploy %, Trade #2 gets half."""    base = tier_lookup(balance, INVEST_TIERS)    return base if trade_number == 1 else base / 2.0

def base_risk_percent(balance):    """Base risk% from tiers (before Trade #2 adjustment)."""    return tier_lookup(balance, RISK_TIERS)
...
# ----------------------------# CORE CALC# ----------------------------
def calc(balance, entry_price, trade_number, target_gain_pct, fee_per_contract):    """    Core sizing + TP/SL math.    All account impact percentages are based on ACCOUNT balance, not contract size.    Instead of always using the maximum contracts, we search all valid contract counts    and pick the one whose NET account gain (after fees) is closest to target_gain_pct.    """
    inv_pct = invest_percent(balance, trade_number)    rsk_pct_base = base_risk_percent(balance)
    # Trade #2 uses half risk budget as well as half deploy    if trade_number == 1:        rsk_pct = rsk_pct_base    else:        rsk_pct = rsk_pct_base / 2.0
    inv_budget = balance * inv_pct / 100.0    rsk_budget = balance * rsk_pct / 100.0
    # SL% on premium    sl_pct = SL_TRADE1_BASE if trade_number == 1 else trade2_dynamic_sl(entry_price)
    cost_per_contract = entry_price * 100.0    sl_price = entry_price * (1.0 - sl_pct / 100.0)    loss_per_contract = (entry_price - sl_price) * 100.0
    # Contract limits from deploy & risk budgets    if cost_per_contract > 0:        max_by_invest = math.floor(inv_budget / cost_per_contract)    else:        max_by_invest = 0
...
    # If nothing fits, return a "zero" plan    if max_contracts == 0:        return {            "contracts": 0,            "inv_pct": inv_pct,            "rsk_pct": rsk_pct,            "sl_pct": sl_pct,            "tp_pct_effective": 0.0,            "pos_cost": 0.0,            "tp_price": entry_price,            "sl_price": sl_price,            "profit_tp_gross": 0.0,            "loss_sl": 0.0,            "total_fees_est": 0.0,            "net_profit_tp": 0.0,            "acct_gain_tp_gross": 0.0,            "acct_loss_sl_gross": 0.0,            "acct_gain_tp_net": 0.0,            "max_by_invest": max_by_invest,            "max_by_risk": max_by_risk,            "inv_budget": inv_budget,            "rsk_budget": rsk_budget,            "cost_per_contract": cost_per_contract,        }
