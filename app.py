...
def compute_tp_price_with_cap(entry_price, tp_cap_pct):
    """
    Convert a TP cap (% move on premium) into a TP price.
...
def compute_tp_percent_for_target_account_gain_net(
    ...
    fee_round_trip_per_contract is TOTAL for one contract: buy + sell.
    """
...
def calc(
    ...
    # Search all n=1..max_contracts and choose closest NET account gain to target under TP cap
    best = None
    for n in range(1, max_contracts + 1):
        ...
        if best is None:
            ...
            if candidate["_diff"] < best["_diff"] - 1e-9:
                ...
                # If nearly same diff, tie-breaker:
                # Trade 1 prefers more contracts; Trade 2 prefers fewer.
    ...
    # If target impossible under cap: compute max achievable NET using max_contracts at cap TP
    n = max_contracts
    ...
    base.update({
        ...
        "feasible": False,
        "note": (
            f"Not feasible to reach {target_gain_pct_net:.2f}% NET with a {tp_cap_pct:.2f}% contract TP cap "
...
.block-container { max-width: 860px; padding-top: 1.3rem; padding-bottom: 2rem; }
...
<b>Note:</b> If you cap the contract move (ex: 3–8%), the target may be impossible when deploy/risk limits prevent buying enough contracts.
...
with r3c1:
    # ✅ Typed TP cap
    tp_cap_pct = st.number_input(
        "Max contract TP % (premium move cap)",
        min_value=0.50,
        max_value=50.00,
        value=8.00,
        step=0.25,
        format="%.2f",
        help="This caps how much the option premium is allowed to increase to reach your TP (per contract).",
    )
with r3c2:
...
m3.metric("TP Cap", f"{tp_cap_pct:.2f}%")
...
copy_text = (
    ...
    f"TP ${res['tp_price']:.2f} (cap {tp_cap_pct:.2f}%) | "
...
st.caption("Not financial advice. Tool is for position sizing / risk math only.")    return tp_price
...
r3c1, r3c2 = st.columns(2)
with r3c1:
    tp_cap_pct = st.selectbox("Max contract TP % (premium move cap)", [3.0, 4.0, 8.0, 10.0, 12.0, 15.0, 18.0, 20.0], index=5)
with r3c2:
    default_target = 0.80 if trade_number == 1 else 0.40
    target_gain_pct = st.slider(
        "Target NET account gain (%)",
